#!/usr/bin/env python3
"""
C4 Calibration Baseline for LRM Pruning.

Runs standard SparseGPT pruning with C4 (generic pre-training text) as
calibration data — the original SparseGPT recipe. This serves as a lower
bound: it shows how much accuracy is lost when using generic text instead
of on-policy reasoning traces (RAC) or outcome-stratified data (OBC-Prune).

Usage:
    python run_c4_baseline.py \
        --model_name deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B \
        --sparsity 0.4 \
        --output_dir ./models/c4_1.5B_sparse40
"""
...
# Reuse the existing TRL SparseGPT infrastructure
from trl.pruner.pruning import sparsegpt_prune, compute_sparsity
from torch.utils.data import DataLoader

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
logger = logging.getLogger("c4_baseline")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="C4 calibration SparseGPT pruning baseline")
    ...
    p.add_argument("--memory_limit_gb", type=float, default=30.0,
                   help="GPU memory budget for Hessian groups (default: 30 GB)")
...
def main() -> None:
    ...
    # ── 3. Prune ───────────────────────────────────────────────────────────
    logger.info(
        f"Starting SparseGPT pruning (C4 calibration) at {args.sparsity*100:.0f}% sparsity"
    )
    t0 = time.time()
