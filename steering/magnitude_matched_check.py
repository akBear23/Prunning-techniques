"""
Symmetric-magnitude follow-up: the original sweeps (sparsity_sweep_1p5b_7b.py,
sparsity_sweep_c4_1p5b_7b.py, sparsity_sweep_gap_40_50.py) used alpha_mult
in {-2, 0, +4} -- an ASYMMETRIC grid inherited from the earlier Llama-8B
fine-grained sweep, never re-validated for 1.5B/7B. This means "steering
toward correct (-2) beats baseline more often than steering toward wrong
(+4) does" is confounded with "+4 is simply a larger-magnitude
perturbation than -2."

This script adds the two missing points needed to form magnitude-matched
pairs without re-running anything already done:
  - alpha_mult = +2   (pairs with the existing -2 -> a matched +-2 comparison)
  - alpha_mult = -4   (pairs with the existing +4 -> a matched +-4 comparison)

Run across ALL checkpoints already tested: dense + all 9 OBC-Prune
sparsities (10-90%) + all 9 C4 sparsities (10-90%), for ONE model size
per invocation (pass size as sys.argv[1]: "1.5B" or "7B"), so the two
sizes can run in parallel on separate GPU jobs.

Which alphas to run is controlled by sys.argv[2], a comma-separated list
(default "2" -- just the +2 point, run first as its own complete pass;
-4 is deferred to a separate later invocation with sys.argv[2]="-4").
...
def main():
    ...
    alphas = [float(a) for a in sys.argv[2].split(",")] if len(sys.argv) > 2 else [2]
    alphas = [int(a) if a == int(a) else a for a in alphas]
    cfg = MODEL_CONFIGS[size_label]
    device = torch.device("cuda")

    print(f"\n\n########################## SIZE {size_label} (symmetric-magnitude check, alphas={alphas}) ##########################",
          flush=True)
    ...
    print(f"\n===== [{size_label}] SYMMETRIC-CHECK DONE =====", flush=True)
